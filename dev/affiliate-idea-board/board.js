(function () {
  ...
  const ASSIGN_PAGE_OPTIONS = [
    ...
    { path: "Desk Setup/setup.html", label: "Apartment tour" },
    ...
    { path: "Photography/photography.html", label: "Photography" },
    { path: "Posts/post-builder.html", label: "Post builder" },
    { path: "Garage Sale/garage-sale.html", label: "Garage sale" },
  ...
  function amazonSearchUrl(query, tag) {
    return (
      "https://www.amazon.com/s?k=" + encodeURIComponent(query) + "&tag=" + encodeURIComponent(tag || "owenminercs-20")
    );
  }
  ...
  function renderSuggestionCard(s, amazonTag, onDoneChange) {
    ...
    media.innerHTML = "<span>Suggestion — add to <code>items.json</code> or <code>generate-items.mjs</code> when ready.</span>";
  ...
  async function main() {
      ...
      copyBtn.addEventListener("click", function () {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(t).then(
            ...
            function () {
              window.alert("Clipboard copy failed and manual text input is currently disabled.");
            }
          );
        } else {
          window.alert("Clipboard API unavailable and manual text input is currently disabled.");
        }
      });
    }
    ...
    function redrawSuggestions() {
      ...
      sugBoard.innerHTML = "";
      list.forEach((s) => sugBoard.appendChild(renderSuggestionCard(s, tag, redrawSuggestions)));
      const hidden = sugHide && sugHide.checked ? rows.filter((r) => localStorage.getItem(SUGGEST_DONE_PREFIX + r.id) === "1").length : 0;
      sugStat.textContent = list.length + " suggestions" + (hidden ? " (" + hidden + " hidden)" : "") + " · " + rows.length + " total";
    }
