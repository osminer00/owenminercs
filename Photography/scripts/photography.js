(function () {
	...
	var lastFocus = null;
	var photoBase =
		typeof window !== 'undefined' && window.OWEN_PHOTOS_BASE
			? String(window.OWEN_PHOTOS_BASE)
			: '';
	var photosJsonUrl =
		typeof window !== 'undefined' && window.OWEN_PHOTOS_JSON
			? String(window.OWEN_PHOTOS_JSON)
			: 'photos.json';

	function resolvePhotoPath(p) {
		if (!p || typeof p !== 'string') return '';
		var t = p.trim();
		if (/^https?:\/\//i.test(t) || t.indexOf('data:') === 0 || t.indexOf('/') === 0) return t;
		return photoBase + t;
	}
	...
	function downloadBasename(photo) {
		...
		var base =
			(photo.title || 'photo').replace(/[^\w\-]+/g, '-').replace(/^-|-$/g, '') || 'photo';
		var full = photo.full || '';
	...
	function openDialog(photo) {
		...
		dlgImg.src = resolvePhotoPath(photo.full || photo.thumb || '');
		...
		dlgDownload.href = resolvePhotoPath(photo.full || photo.thumb) || '#';
		dlgDownload.setAttribute('download', downloadBasename(photo));

		var printUrl;
		if (
			Object.prototype.hasOwnProperty.call(photo, 'signedPrintUrl') &&
			photo.signedPrintUrl != null
		) {
	...
	function renderYearSection(year, photos) {
		...
		photos.forEach(function (photo) {
			var generatedAlt =
				window.AccessibleMedia && typeof window.AccessibleMedia.describePhoto === 'function'
					? window.AccessibleMedia.describePhoto(photo, year)
					: photo.alt || photo.title || '';
			var generatedCaption =
				window.AccessibleMedia && typeof window.AccessibleMedia.captionPhoto === 'function'
					? window.AccessibleMedia.captionPhoto({
							title: photo.title,
							alt: generatedAlt,
			...
			img.src = resolvePhotoPath(photo.thumb || photo.full || '');
	...
	fetch(photosJsonUrl)
		.then(function (r) {
			if (!r.ok) throw new Error('Could not load ' + photosJsonUrl);
			return r.json();
		})
		.then(function (data) {
		...
		.catch(function () {
			if (errEl) {
				errEl.hidden = false;
				errEl.textContent =
					'Could not load the photo list. Check that ' +
					photosJsonUrl +
					' is reachable (paths use OWEN_PHOTOS_JSON / OWEN_PHOTOS_BASE on embed pages).';
			}
