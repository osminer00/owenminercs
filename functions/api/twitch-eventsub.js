...
function statCommandsForEvent(event) {
  ...
  if (event.type === "bits") commands.push(["HINCRBY", TOTALS_HASH_KEY, "bits_total", String(event.bits || 0)]);
  return commands;
}
...
export async function onRequestPost(context) {
  ...
  try {
    const idempotentResult = await upstashCommand(env, ["SET", idempotencyKey, "1", "NX", "EX", "86400"]);
    ...
    return json({ error: "Failed to persist Twitch event.", detail: String(error.message || error) }, 500);
  }
