...
async function fetchYouTubeViaDataApi(apiKey, channelId, limit) {
	...
	while (videoIds.length < targetCount) {
		...
		if (!searchResponse.ok) {
			...
			throw new Error(
				`YouTube search failed (${searchResponse.status}): ${body.slice(0, 220)}`
			);
		}
	...
	for (let index = 0; index < videoIds.length; index += 50) {
		...
		if (!videosResponse.ok) {
			...
			throw new Error(
				`YouTube videos lookup failed (${videosResponse.status}): ${body.slice(0, 220)}`
			);
		}
...
async function resolveChannelIdFromHandle(handleOrUsername) {
	const cleaned = String(handleOrUsername || '')
		.replace(/^@/, '')
		.trim();
	if (!cleaned) return '';
...
async function getFeedPayload(env, limit) {
	...
	const youtubeUsername = String(env?.YOUTUBE_USERNAME || env?.YOUTUBE_HANDLE || 'OwenMinerCS')
		.replace(/^@/, '')
		.trim();
	...
	if (usingDataApi) {
		if (!channelId) {
			...
			try {
				...
				warnings.push(
					`Data API failed, falling back to RSS: ${String(error?.message || error)}`
				);
			}
		}
	}
