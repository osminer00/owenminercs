...
- Donator “Recent Twitch activity” is paused; `fetchTwitchFeed()` is a stub. EventSub Functions still exist.
- `GET /api/social-feed` and `GET /api/live-status` have no in-repo page clients.
- `scripts/ai-assistant.js` has no HTML host; `/assistant` redirects home.
- `Music/music.html` exists but public `/music` URLs redirect home.
...
- Netlify/API code may exist in both `functions/api/` and `netlify/functions/`; check both before changing behavior. `social-feed` has no Netlify twin.
- `QA/scripts/qa-feed.js` skips `/api/discord-qa` on localhost; use static `QA/answered-qa.json` or a Functions deploy.
- Twitch EventSub on `main` SETs `activity:twitch:seen:` NX before LPUSH. A persist failure after that SET can drop retries. Unmerged “critical bug” PRs are not shipped.
- `scripts/sync-x-top-posts.py` always overwrites `Socials/data/x-top-posts.json` (including `[]`).
