...
- Shared nav `data-nav` values may not match URL casing (for example `data-nav="Dev"` vs `/dev/`); keep active-link and nav-tour mappings explicit and covered by route regression tests.
