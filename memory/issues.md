...
- Event ingestion idempotency must be atomic with persistence. Do not mark Twitch EventSub messages as seen before the event/counter writes complete; use the Redis Lua `EVAL` path in both `functions/api/twitch-eventsub.js` and `netlify/functions/twitch-eventsub.js`.
