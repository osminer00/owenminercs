...
- Keyboard hub is intentional: `Keyboard/60he.html` chooses `60he-2025` / `60he-2023` (see `KEYBOARD_HUB_RUNBOOK.md`).
...
- X/Twitter handle on public surfaces: `@OwenMiner` / `https://x.com/OwenMiner` (not `@owenminercs`). Sync script: `X_TOP_POSTS_SYNC_RUNBOOK.md`.
...
- `SOCIAL_DOCK_RUNBOOK.md` — floating social dock ops / QA + current profile URL table.
...
- `KEYBOARD_HUB_RUNBOOK.md` — Wooting hub vs year pages; do not restore monolith casually.
- `X_TOP_POSTS_SYNC_RUNBOOK.md` — Social Cloud X top-posts sync + empty-overwrite pitfall.
