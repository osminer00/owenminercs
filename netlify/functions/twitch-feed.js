...
exports.handler = async function handler(event) {
  ...
  const limit = Math.min(parsePositiveInt(event.queryStringParameters?.limit, DEFAULT_LIMIT), MAX_LIMIT);
  ...
  try {
    ...
    const lastUpdated = await upstashCommand(["GET", "activity:twitch:last_updated"]).catch(() => null);
    ...
    return json(500, { error: "Failed to load Twitch feed.", detail: String(error.message || error) });
  }
};
