...
async function resolveSteamId64(profile) {
	...
	if (!steamId64) {
		throw new Error(`Could not resolve Steam profile: ${profile}`);
	}
	return steamId64;
}
...
async function enrichItemsWithPricing(items) {
	...
	return items.map((item) => {
		const pricing = pricingMap.get(item.marketHashName) || null;
		return {
			...item,
			pricing,
		};
...
function mapInventoryItems(payload, ownerSteamId) {
	...
	return assets.map((asset) => {
		...
		const icon = desc.icon_url ? `https://community.cloudflare.steamstatic.com/economy/image/${desc.icon_url}/360fx360f` : null;
		...
		return {
			...
			iconUrl: icon,
			inspectLink,
			inspectActionName: inspectAction?.name || null,
...
exports.handler = async function handler(event) {
	...
	try {
		const profile = getQueryParam(event, 'profile') || DEFAULT_PROFILE;
		const limit = Math.max(1, Math.min(asNumber(getQueryParam(event, 'limit'), 120), 300));
		const perPage = Math.max(1, Math.min(asNumber(getQueryParam(event, 'count'), 120), MAX_FETCH_COUNT));
		const expensiveMin = Math.max(0, asNumber(getQueryParam(event, 'expensiveMin'), DEFAULT_EXPENSIVE_MIN));
		const featuredOnly = getQueryParam(event, 'featured') !== '0';
