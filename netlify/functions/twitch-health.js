...
exports.handler = async function handler(event) {
  ...
  if (!missing.length) {
    try {
      ...
      response.checks.upstash = { ok: String(upstash).toUpperCase() === "PONG", skipped: false };
    } catch (error) {
    ...
    try {
      response.checks.twitchAuth = {
        ...(await checkTwitchAuth(process.env.TWITCH_CLIENT_ID, process.env.TWITCH_CLIENT_SECRET)),
        skipped: false
      };
    } catch (error) {
      response.checks.twitchAuth = {
