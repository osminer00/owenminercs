(function () {
    ...
    return Array.prototype.slice
      ...
      .map(function (card) {
        ...
        return {
          ...
          thumbSrc: thumbEl && thumbEl.getAttribute('src') ? thumbEl.getAttribute('src') : '',
          thumbAlt: thumbEl && thumbEl.getAttribute('alt') ? thumbEl.getAttribute('alt') : '',
  ...
  function createSummary(a, b) {
    ...
    if (keySpec) {
      ...
      summary = 'These are different tools for different jobs, so choose based on your workflow and camera/audio/desk goals.';
    }
  ...
  function renderComparisons(items) {
    ...
    allCards.sort(function (left, right) {
      function score(pair) {
        ...
        var opinions =
          (pair.thisItem.opinion ? 1 : 0) + (pair.thatItem.opinion ? 1 : 0);
    ...
    allCards.slice(0, 6).forEach(function (pair) {
      ...
      article.innerHTML =
        ...
        (opinionsMarkup ? '<ul class="setup-vs-card__opinions">' + opinionsMarkup + '</ul>' : '') +
