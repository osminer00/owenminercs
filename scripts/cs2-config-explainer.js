(() => {
  const KNOWN_COMMANDS = {
    sensitivity: {
      ...
      check: numericRangeCheck(0.4, 3.0, "Common range is about 0.4 to 3.0 for most players.")
    },
    zoom_sensitivity_ratio: {
      description: "Adjusts scoped sensitivity relative to your base sensitivity.",
      check: numericRangeCheck(0.5, 1.5, "Most players keep this near 1.0 (usually 0.5 to 1.5).")
    },
    fps_max: {
      description: "Caps your maximum in-game FPS.",
      check: numericRangeCheck(120, 1000, "Very low or very high FPS caps can feel odd. Typical range is 120 to 1000.")
    },
    rate: {
      description: "Sets max network bandwidth usage for server traffic.",
      check: numericRangeCheck(128000, 1000000, "Unusual rate value. Common modern values are 196608 or higher.")
    },
    cl_updaterate: {
    ...
    cl_crosshairthickness: {
      ...
      check: numericRangeCheck(0.1, 3.0, "Crosshair thickness outside 0.1 to 3.0 is uncommon.")
    },
    cl_crosshairgap: {
    ...
    m_yaw: {
      ...
      check: numericRangeCheck(0.01, 0.05, "Default is 0.022. Large changes can break your feel.")
    },
    m_pitch: {
    ...
    sv_cheats: {
      ...
      check: (value) => value === "1" ? "This enables cheats and will not work in normal VAC secured matchmaking." : ""
    },
    cl_interp: {
      description: "Legacy interpolation command from older Counter-Strike versions.",
      check: () => "This command is legacy/limited in CS2 and may not behave like it did in CS:GO."
    },
    cl_interp_ratio: {
  ...
  function analyzeLine(rawLine, lineNumber) {
    ...
    const item = {
      ...
      command,
      value,
      explanation: rule ? rule.description : "Custom or less common command. Check Valve docs or test manually.",
      warning: ""
    };
