(function () {
	...
	function buildHashtags(topic, keywords) {
		...
		base.forEach(function (entry) {
			var compact = String(entry || '')
				.toLowerCase()
				.replace(/[^a-z0-9 ]+/g, ' ');
			var tag = compact.split(/\s+/).filter(Boolean).slice(0, 3).join('');
	...
	function generateCopyDraft(prompt, tone, keywords) {
		...
		var introsByTone = {
			balanced:
				'New post: ' +
				normalized +
				'. Sharing a quick look at what changed and why it matters.',
			casual:
				'Quick update: ' +
				normalized +
				'. Dropping a short breakdown and fresh shots in this post.',
			hype:
				normalized +
				' just dropped. New post is up with the full breakdown, details, and photos.',
		};
		...
		var seoDescBits = [
			...
			normalized + '.',
			keyList.length
				? 'Includes ' + keyList.join(', ') + '.'
				: 'Includes photos and a quick summary.',
		];
	...
	document.getElementById('pb-add-post').addEventListener('click', function () {
		...
		setStatus(
			'Saved draft in memory: "' +
				title +
				'" — ' +
				posts.length +
				' post(s) total. Download JSON to update the site.'
		);
		updatePreview(post);
		resetForm();
	});
	...
	if (aiPromptEl && aiIntroEl && aiSeoTitleEl && aiSeoDescEl && aiSocialEl) {
		document.getElementById('pb-ai-generate').addEventListener('click', function () {
			...
			var draft = generateCopyDraft(
				prompt,
				(aiToneEl && aiToneEl.value) || 'balanced',
				aiKeywordsEl && aiKeywordsEl.value
			);
		...
		document.getElementById('pb-ai-copy-all').addEventListener('click', function () {
			...
			copyTextToClipboard(
				...
				function () {
					setAiStatus(
						'Could not copy automatically. Select and copy the fields manually.'
					);
				}
			);
		});
	...
	resetForm();
	setStatus(
		'Load your current posts.json (optional), fill the form, click Add post, then Download posts.json.'
	);
