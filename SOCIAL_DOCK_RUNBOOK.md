...
X handle on public surfaces is `@OwenMiner` / `https://x.com/OwenMiner` (not `@owenminercs`). Local Social Cloud X sync (`scripts/sync-x-top-posts.py`) defaults to `DEFAULT_USERNAME = "OwenMiner"` and should stay aligned with the dock link. See `X_TOP_POSTS_SYNC_RUNBOOK.md`.
