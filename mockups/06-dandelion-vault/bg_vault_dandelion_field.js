(function () {
	...
	const controlKeys = sharedBackgrounds.controlKeys || [
		'speed',
		'density',
		'scale',
		'glow',
	...
	const fallbackControls = sharedBackgrounds.fallbackControls || {
		speed: 1,
		density: 1,
		scale: 1,
		glow: 1,
	...
	const cloneControls =
		sharedBackgrounds.cloneControls ||
		function (source) {
			const next = {};
	...
	const sceneConfigs = sharedBackgrounds.createLabSceneConfigs
		? sharedBackgrounds.createLabSceneConfigs()
		: [];
	...
	const presets =
		Array.isArray(scene.presets) && scene.presets.length
			? scene.presets
			: [
					{
						id: 'balanced',
	...
	let animationSpeed = clamp(
		typeof storedState.animationSpeed === 'number' ? storedState.animationSpeed : 0.2,
		0.2,
		2.4
	);
	let horizontalPosition = clamp(
		typeof storedState.horizontalPosition === 'number' ? storedState.horizontalPosition : 0.23,
		0.12,
		0.6
	);
	let verticalPosition = clamp(
		typeof storedState.verticalPosition === 'number' ? storedState.verticalPosition : 0.285,
		0.16,
		0.72
	);
	...
	let rafId = 0;
	const glowSprites = {};
	let glowSpriteCount = 0;
	let breeze = { phase: Math.random() * Math.PI * 2, offsetX: 0, offsetY: 0 };

	const initialPreset =
		findPresetById(forcedPresetParam) || findPresetById(storedState.presetId) || presets[0];
	...
	controlKeys.forEach(function (key) {
		sliderEls[key].addEventListener('input', function () {
			controls[key] = clamp(
				parseFloat(sliderEls[key].value) || 1,
				parseFloat(sliderEls[key].min),
				parseFloat(sliderEls[key].max)
			);
			seedData();
			updateControlUI();
	...
	function updateControlUI() {
		...
		if (matchedId === 'custom-live') {
			...
			presetHint.textContent =
				'The sliders are in a custom live state for this standalone page.';
		} else {
	...
	function resize() {
		...
		Object.keys(glowSprites).forEach(function (k) {
			delete glowSprites[k];
		});
		glowSpriteCount = 0;
		const rect = stage.getBoundingClientRect();
	...
	function drawGlow(context, x, y, radius, color, alpha) {
		const colorFilled = color.replace('ALPHA', alpha.toFixed(3));
		const key = Math.round(radius) + '_' + colorFilled;
		let sprite = glowSprites[key];
		if (!sprite) {
			...
			g.addColorStop(0, colorFilled);
			g.addColorStop(1, 'rgba(0,0,0,0)');
			sc.fillStyle = g;
			sc.fillRect(0, 0, dim, dim);
			if (glowSpriteCount < 48) {
				glowSprites[key] = sprite;
				glowSpriteCount++;
			}
		}
		const d = radius * 2;
		context.drawImage(sprite, x - radius, y - radius, d, d);
	}
	...
	function drawDandelionSeed(context, originX, originY, angle, travel, depth, alpha) {
		...
		const pointerPush =
			pointerDist < 180
				? (1 - pointerDist / 180) * 26 * controls.parallax * (0.45 + pointer.energy)
				: 0;
		...
		drawSeedCrown(
			...
			fluffX,
			fluffY,
			2.4 + depth * 0.85 + travel * 1.4,
			angle + time * 0.08,
			Math.min(0.96, alpha + 0.1)
		);
		drawGlow(
			context,
			fluffX,
	...
	function drawWideDandelionSeedBurst(context, x, y, radius, phase, pulse, alphaBoost, depth) {
		...
		const attachedSeeds = Math.floor(
			clamp(14 + controls.density * 10 + depth * 4, 14, 28) * (1 - release * 0.78)
		);
		...
		drawGlow(
			...
			x,
			y,
			radius * 2.0 * controls.glow,
			'rgba(255, 248, 214, ALPHA)',
			(0.06 + whitening * 0.05) * controls.glow * alphaBoost
		);

		for (let i = 0; i < attachedSeeds; i += 1) {
			const angle =
				(i / Math.max(1, attachedSeeds)) * Math.PI * 2 +
				pulse +
				Math.sin(i * 2.1 + pulse) * 0.045;
	...
	function drawDandelionSeedArm(
		...
		baseContext.lineWidth = Math.max(
			1.1,
			thickness * (0.72 + depthLeft * 0.16) * controls.scale
		);
		baseContext.strokeStyle =
			'rgba(176,255,226,' +
			Math.max(0.12, 0.12 + depthLeft * 0.028 * controls.glow).toFixed(3) +
			')';
		...
		glowContext.strokeStyle =
			'rgba(224,255,241,' +
			Math.max(0.1, 0.08 + depthLeft * 0.022 * controls.glow).toFixed(3) +
			')';
		...
		drawDandelionSeedArm(
			baseContext,
			glowContext,
			nx,
			ny,
			length * 0.74,
			angle - split + sway + pointerBias + bend,
			depthLeft - 1,
			sway,
			phase + 0.015,
			thickness * 0.82
	...
	function drawDandelionSupportArm(
		baseContext,
		glowContext,
		x,
		y,
		...
		baseContext.lineWidth = Math.max(
			0.8,
			thickness * (0.6 + depthLeft * 0.14) * controls.scale
		);
		baseContext.strokeStyle =
			'rgba(176,255,226,' +
			Math.max(0.08, 0.08 + depthLeft * 0.02 * controls.glow).toFixed(3) +
			')';
		baseContext.beginPath();
		baseContext.moveTo(x, y);
		baseContext.lineTo(nx, ny);
		...
		glowContext.lineWidth = Math.max(
			0.65,
			thickness * (0.34 + depthLeft * 0.08) * controls.scale
		);
		glowContext.strokeStyle =
			'rgba(224,255,241,' +
			Math.max(0.06, 0.06 + depthLeft * 0.016 * controls.glow).toFixed(3) +
			')';
		glowContext.beginPath();
		glowContext.moveTo(x, y);
		glowContext.lineTo(nx, ny);
	...
	function drawDandelion() {
		...
		const pointerForce =
			pointerDist < 240
				? (1 - pointerDist / 240) * 0.22 * controls.parallax * (0.55 + pointer.energy)
				: 0;
		...
		drawGlow(
			...
			pointer.x,
			pointer.y,
			180 * controls.glow * (1 + pointer.energy),
			'rgba(255, 244, 209, ALPHA)',
			0.06 * controls.glow
		);
		...
		for (let j = 0; j < upperArmCount; j += 1) {
			...
			const upperAngle =
				upperFan + sway * 0.65 + (pointerDx / pointerDist) * pointerForce * 0.55;
		...
		[Math.PI, 0].forEach(function (centerAngle, sideIndex) {
			for (let m = 0; m < sideArmCount; m += 1) {
				...
				const sideAngle =
					...
					sideFan +
					sway * 0.56 +
					(pointerDx / pointerDist) * pointerForce * 0.48;
				const sideLength =
					baseLength * (0.88 + Math.sin(m * 1.12 + time * 0.13 + sideIndex) * 0.06);
				drawDandelionSeedArm(
					ctx,
					ptx,
					anchorX,
					...
					4,
					sway * 0.54,
					phase + 0.34 + sideIndex * 0.1 + m * 0.013,
					3.1
				);
			}
		});

		const supportArmCount =
			controls.density > 1.5
				? Math.floor(clamp(12 + densityFactor * 8, 12, 24))
				: Math.floor(clamp(18 + densityFactor * 12, 18, 36));
		...
		drawWideDandelionSeedBurst(
			...
			anchorX,
			anchorY,
			Math.min(size.width, size.height) * 0.12 * controls.scale,
			phase,
			time * 0.14,
			1,
			1.1
		...
		trunkMotes.forEach(function (mote, index) {
			...
			const alpha = clamp(
				0.08 + Math.sin(time * 1.7 + mote.pulse + index * 0.2) * 0.06,
				0.05,
				0.18
			);
			drawDandelionSeed(
	...
	function renderFrame(timestamp) {
		...
		const rawDelta = lastTimestamp ? (timestamp - lastTimestamp) / 1000 : 1 / 60;
		lastTimestamp = timestamp;
		const dt = Math.min(rawDelta, 0.05) * 60;
		time += dt * 0.016 * animationSpeed;

		// Breeze simulation (ambient, non-pointer-driven)
		// offsets vary slowly with time and scale with controls.breeze
		const breezeStrength = clamp(
			typeof controls.breeze === 'number' ? controls.breeze : 0.2,
			0,
			2
		);
		...
		pointer.energy +=
			(Math.min(1.4, motionMag / 18 + breezeStrength * 0.24) - pointer.energy) * 0.14;
		pointer.lx = pointer.x;
		pointer.ly = pointer.y;
