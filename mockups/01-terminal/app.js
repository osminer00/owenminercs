  ...
  function matrix() {
    ...
    matrixTimer = setInterval(() => {
      ...
      for (let i = 0; i < Math.min(cols, 48); i++) {
        row += Math.random() > 0.92 ? String.fromCharCode(0x30a0 + Math.random() * 96) : ["0", "1"][Math.random() > 0.5 ? 0 : 1];
      }
      line(row);
      t++;
      if (t > 18) {
  ...
  form.addEventListener("submit", (e) => {
    ...
    switch (cmd) {
      ...
      case "theme":
        ...
        else if (arg === "green") { /* default */ }
        else line("usage: theme green | theme amber", "err");
        break;
      case "goto":
        if (arg === "about") document.getElementById("about")?.scrollIntoView({ behavior: "smooth" });
