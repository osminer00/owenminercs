...
function getProgramsNavHrefs() {
	return Array.from(
		componentsSource.matchAll(
			/<a href="\$\{getLink\('(?<path>dev\/dev-stack)'\)\}" class="site-nav-link" data-nav="Dev" title="Programs for coding, creative work, and streaming">Programs<\/a>/g
		),
...
test('Programs nav entries point to the deployed lowercase dev stack route', () => {
	const hrefs = getProgramsNavHrefs();

	assert.equal(hrefs.length, 2, 'header and footer should both expose the Programs nav item');
	for (const href of hrefs) {
		assert.equal(href, 'dev/dev-stack');
		assert.ok(existsSync(resolve(repoRoot, `${href}.html`)), `${href}.html should exist`);
	}
});
